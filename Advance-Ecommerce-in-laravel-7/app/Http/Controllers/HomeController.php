...
class HomeController extends Controller
{
    ...
    public function userOrderDelete($id)
    {
        ...
        if($order){
           if($order->status=="process" || $order->status=='delivered' || $order->status=='cancel'){
                return redirect()->back()->with('error','Bạn không thể xóa khóa học này bây giờ');
           }
           else{
                $status=$order->delete();
                if($status){
                    ...
                    request()->session()->flash('error','Không thể xóa khóa học');
                }
                return redirect()->route('user.order.index');
           }
        }
        else{
            request()->session()->flash('error','Không thể tìm thấy đơn đặt');
            return redirect()->back();
        }
    }
    ...
    public function userCommentDelete($id){
        ...
        if($comment){
            ...
            if($status){
                request()->session()->flash('success','Bài viết bình luận đã được xóa thành công');
            }
            else{
                request()->session()->flash('error','Đã xảy ra lỗi vui lòng thử lại');
            }
