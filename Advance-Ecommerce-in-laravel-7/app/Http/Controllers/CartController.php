...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request){
        ...
        if (empty($request->slug)) {
            request()->session()->flash('error','Khóa học không hợp lệ!');
            return back();
        }
        ...
        request()->session()->flash('success','Khóa học được thêm vào giỏ hàng thành công');
        return back();
    }
    ...
    public function singleAddToCart(Request $request){
        ...
        if($product->stock <$request->quant[1]){
            return back()->with('error','Lớp đã đầy. Bạn có thể bổ sung khóa học khác.');
        }
            ...
            foreach ($request->quant as $k=>$quant) {
                ...
                if($quant > 0 && $cart) {
                    ...
                    if($cart->product->stock < $quant){
                        request()->session()->flash('error','Lớp đã full');
                        return back();
                    }
                    ...
                    // return $cart->price;
                    $cart->save();
                    $success = 'Đã cập nhật thành công giỏ hàng!';
                }else{
